...
string FindTownOnIsland(string sIsland)
{
	for (int i = 0; i < MAX_COLONIES; i++)
	{
		if (Colonies[i].island == sIsland)
		{
...
//Hokkins: взял эти методы из шип пака -->
...
int GetMaxDaysFromIsland2Island(string sFromIsland, string sToIsland)
{
	if(sFromIsland == "" || sToIsland == "")
	{
		return -1;
	}
	string sFromTown = FindTownOnIsland(sFromIsland);
	string sToTown = FindTownOnIsland(sToIsland);
	int _dist = CalculateColonyDistance(sFromTown, sToTown);
	return _dist + 2;
}
//Hokkins: <--