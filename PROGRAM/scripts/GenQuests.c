...
void ReasonToFast_GenerateHunter() // генерация ОЗГ
{
	...
	chr = GetCharacter(NPC_GenerateCharacter("ReasonToFast_Hunter", "officer_"+(rand(19)+1), "man", "man", iRank, PIRATE, -1, true));
...
void ReasonToFast_GenerateVictimShip(int iShipType, int iShipGoods) // генерация призового корабля
{
	...
	ref chref = GetCharacter(NPC_GenerateCharacter("VictimCap_1", "officer_"+(rand(19)+1), "man", "man", 25, iNation, -1, true));
...
void ReasonToFast_PreparePirateShip(string qName) // генерация пиратского корабля
{
	...
	sld = GetCharacter(NPC_GenerateCharacter("PirateCapt", "officer_"+(rand(19)+1), "man", "man", iRank, PIRATE, 30, true));	
	sld.dialog.filename      = "GenQuests_Dialog.c";
...
void EncGirl_SetBerglar(string qName)
{
	...
	ref sld = GetCharacter(NPC_GenerateCharacter("Berglar_EncGirl", "officer_"+(rand(19)+1), "man", "man", iTemp, PIRATE, -1, true));
...
{
	int iTemp = sti(pchar.rank) + rand(5);
	ref sld = GetCharacter(NPC_GenerateCharacter("Lover_EncGirl", "officer_"+(rand(19)+1), "man", "man", iTemp, sti(pchar.GenQuest.EncGirl.nation), -1, true));
...
void CaptainComission_GeneratePirateShips(string qName)
{
	...
	for (i=1; i<=2; i++)
	{
		...
		sld = GetCharacter(NPC_GenerateCharacter("CaptainAttack_"+i, "officer_"+(rand(19)+1), "man", "man", Rank, PIRATE, 3, true));
...
void CaptainComission_ChangeCaptain()
{
	...
	chref = GetCharacter(NPC_GenerateCharacter("CapComission_2", "officer_"+(rand(19)+1), "man", "man", iRank, iNation, -1, true));
...
void CaptainComission_EnterToSeaAfterShoreBattle()
{
	...
	for (int i = 1; i <= 3; i++)
    {
		sld = GetCharacter(NPC_GenerateCharacter("Pirate_Captain0" + i, "officer_"+(rand(19)+1), "man", "man", 5, PIRATE, 3, true)); 
