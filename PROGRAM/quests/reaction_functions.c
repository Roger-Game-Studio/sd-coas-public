...
void Slavetrader_CreateSlaveShips(string qName)//создание кораблей в бухте
{
	...
	for (i=1; i<=2; i++)//постоянно такая структура
	{
		...
		sld = GetCharacter(NPC_GenerateCharacter("CaptainSlaveAttack_"+i, "officer_"+(rand(19)+1), "man", "man", Rank, PIRATE, 3, true));//создание кэпа
		if (i == 1)
		{
...
void Slavetrader_EscapeSlaveInShore(string qName)
{
    ...
    {
        iTemp = 10 + rand(10);
		sld = GetCharacter(NPC_GenerateCharacter("OwnCrew_"+i, "pirate_"+(rand(8)+2), "man", "man", iTemp, sti(pchar.nation), 0, true));
        LAi_SetWarriorType(sld);
		LAi_warrior_DialogEnable(sld, false);
        LAi_group_MoveCharacter(sld, LAI_GROUP_PLAYER);
		sTemp = LAi_FindNearestFreeLocator("goto", locx, locy, locz);
...
void CreateMiko()//создадим Мико Пистольеро
{
	...
	{
		sld = GetCharacter(NPC_GenerateCharacter("AddPearlguard"+i, "Pirate_"+(rand(9)+1), "man", "man", 25, SPAIN, -1, true));
